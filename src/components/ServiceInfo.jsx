            ...
            <div className="flex flex-row mx-30">
                <div className="flex flex-col items-center">
                    {SERVICE_INFO_LIST.map((item) => (
                        <div key={item.id} className={`ServiceInfoItemContent flex flex-col w-[75%] py-5 pl-5 text-black cursor-pointer ${currentSlide === item.id - 1 ? "border-l-4 border-red-500" : "border-l-4 border-gray-300"}`}>
                            <p className="text-2xl font-bold text-[#011E56]">{item.title}</p>
                            <p className="text-gray-700">{item.description}</p>
                            <p className="text-red-500 text-red hover:text-red-700 underline">
                                {FIND_OUT_MORE}
                            </p>
                        </div>
                    ))}
